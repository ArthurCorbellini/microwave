...
- **`scripts/setup-devcontainer.sh`** — detects Docker or Podman and generates `.devcontainer/devcontainer.json` from the matching template. Must be run manually once before opening the project in a Dev Container (it generates the very file the Dev Containers extension needs, so it can't be a `postCreateCommand`/`initializeCommand` inside that file — chicken-and-egg). See `docs/development-setup.md` for the rest of the Dev Container setup, including personal VS Code settings that can't be committed to this repo.
