...
Living description of the system's macro topology — how services, brokers, and (once they exist) the Gateway/BFF connect and talk to each other. This is the "how the pieces fit together" (static) view; [`docs/conventions.md`](conventions.md) is the "how each service is built inside" view, [`docs/roadmap.md`](roadmap.md) is the "what's done and what's next" view, and [`docs/sagas/`](sagas/) is the "what does this trigger set off" (temporal) view — a sequence diagram per saga, tracing one request end to end across the services this file shows connected.
