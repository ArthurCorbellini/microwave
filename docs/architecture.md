...
### Order lifecycle (Phase 4 saga)

The flowchart above shows *which* services talk to each other; it doesn't show *when*. This sequence diagram traces one order through the full choreographed saga — `orders` reacts to each reply and decides its own next step, with no central coordinator:
