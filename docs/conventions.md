...
App services (`catalog`, `orders`, `payments`, `inventory`, `notifications`) run `replicas: 2` behind a `NodePort` Service, on the same host-facing ports `docker-compose.yml` already publishes. Databases, RabbitMQ, and Kafka run `replicas: 1` — no `StatefulSet`, since none of them cluster — behind a `ClusterIP` Service (RabbitMQ additionally gets a `NodePort` Service for its management UI). Every single-replica stateful component (the 5 Postgres Deployments, RabbitMQ, and Kafka) uses `strategy: type: Recreate` — for Postgres this avoids two Pods mounting the same `ReadWriteOnce` volume at once; for RabbitMQ and Kafka (no volume, but still single-instance and non-clustered) it avoids a default `RollingUpdate` briefly running two independent, out-of-sync brokers behind the same Service.
...
- **Per-queue retry interceptor**: `RepublishMessageRecoverer` is constructed with a fixed target routing key, so a service that owns two or more queues must give each queue its own `StatelessRetryOperationsInterceptor` + `SimpleRabbitListenerContainerFactory` pair (named after the queue, e.g. `reserveStockRetryInterceptor`/`reserveStockListenerContainerFactory`), each recoverer targeting that queue's own DLX routing key — sharing one interceptor across queues would misroute the second queue's dead letters into the first queue's DLQ. Every `@RabbitListener` must explicitly name its `containerFactory`; a listener that omits it falls back to whatever un-intercepted default factory Spring Boot auto-configures, which has no retry/dead-letter behavior at all. A service with only one owned queue (e.g. `payments`, currently) may use a single generically-named pair, but must split it into per-queue pairs the moment a second queue is added — see `orders`' and `inventory`'s `RabbitMQConfig` for the pattern to copy.
